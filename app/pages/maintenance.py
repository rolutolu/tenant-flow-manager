    ...
    with page_layout(title="Maintenance Hub"):
        ...
        with ui.dialog() as new_ticket_dialog, ui.card().classes("p-6 w-96"):
            ...
            async def save_ticket():
                ...
                ok = await run.io_bound(add_maintenance_request, user_id, t_issue.value, t_urgency.value, tenant_id, unit_id)
                if ok:
                    ...
                    for t in tickets:
                        ...
                        with ui.expansion(t["issue"][:40], icon="build").classes(
                            ...
                            with ui.column().classes("p-3 gap-2 w-full"):
                                ...
                                async def save_details(tid=t["id"], c=contractor_input, cost=cost_input):
                                    ok = await run.io_bound(update_maintenance_request, tid, contractor=c.value, cost=cost.value or 0)
                                    if ok:
                                ...
                                with ui.row().classes("w-full justify-end gap-1"):
                                    if t["status"] != "Resolved":
                                        async def mark_done(tid=t["id"]):
                                            await run.io_bound(update_maintenance_status, tid, "Resolved")
                                            ui.navigate.to("/maintenance")
                                    ...
                                    if t["status"] == "Open":
                                        async def mark_prog(tid=t["id"]):
                                            await run.io_bound(update_maintenance_status, tid, "In Progress")
                                            ui.navigate.to("/maintenance")
